 ...
 * Solution description: Dynamic Array, Stack and Bag function implementations */
...
void _dynArrSetCapacity(DynArr *v, int newCap) {
	...
	assert (v != NULL);
	TYPE* newMemory = (TYPE *)malloc(sizeof(TYPE) * newCap);

	for (int index = 0; index <= v->size; index++){
		newMemory[index] = v->data[index];
	}
	free(v->data);
	v->data = newMemory;
	v->capacity = newCap;
...
void removeAtDynArr(DynArr *v, int idx) {
	...
	for (int index = idx; index < v->size-1; index++){
